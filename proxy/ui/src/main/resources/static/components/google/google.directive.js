...
(function() {
	...
	angular
		...
		.directive('googlePlusSignin', ['$window', googlePlusSignin ])
		.run(['$window','$rootScope', runGooglePlusDirective ]);
	
	function googlePlusSignin($window) {
	...
	function runGooglePlusDirective($window, $rootScope) {
		$window.googlePlusSigninCallback = function( authResult ) {
			if( authResult && authResult.access_token && !authResult.error ) {
